...
public interface AgentExecutor {
	...
	enum Serializers {
		...
		STD(new LC4jStateSerializer<>(State::new)), JSON(new LC4jJacksonStateSerializer<>(State::new));
	...
	/**
	 * The ExecuteTools class implements the NodeAction interface for handling actions
	 * related to executing tools within an agent's context.
	 */
	class ExecuteTools implements NodeAction<State> {
		...
		public Map<String, Object> apply(AgentExecutor.State state) {
			...
			// .orElseThrow(() -> new IllegalArgumentException("no tool execution request
			// found!"));
			...
			var result = toolExecutionRequests.get()
				.stream()
	...
	/**
	 * The CallAgent class implements the NodeAction interface for handling actions
	 * related to an AgentExecutor's state.
	 */
	class CallAgent implements NodeAction<State> {
		 ...
		 * @throws IllegalStateException if the finish reason of the response is
		 * unsupported
		 */
		...
		public StateGraph<State> build() throws GraphStateException {
			...
			if (streamingChatModel != null && chatModel != null) {
				throw new IllegalArgumentException(
						"chatLanguageModel and streamingChatLanguageModel are mutually exclusive!");
			}
			if (streamingChatModel == null && chatModel == null) {
			...
			final EdgeAction<State> shouldContinue = (
					state) -> state.finalResponse().map(res -> "end").orElse("continue");

			return new StateGraph<>(State.SCHEMA, stateSerializer).addNode("agent", node_async(callAgent))
				.addNode("action", node_async(executeTools))
				.addEdge(START, "agent")
				.addConditionalEdges("agent", edge_async(shouldContinue), Map.of("continue", "action", "end", END))
