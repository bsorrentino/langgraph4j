...
/**
 * The UserMessageSerializer class implements the NullableObjectSerializer interface for
 * the UserMessage type. It provides methods to serialize and deserialize UserMessage
 * objects.
 */
public class UserMessageSerializer implements NullableObjectSerializer<UserMessage> {
	 ...
	 * @throws IllegalArgumentException if the content type of the UserMessage is
	 * unsupported
	 */
	@Override
	...
	public UserMessage read(ObjectInput in) throws IOException, ClassNotFoundException {
		...
		return readNullableUTF(in).map(name -> UserMessage.from(name, text)).orElseGet(() -> UserMessage.from(text));
