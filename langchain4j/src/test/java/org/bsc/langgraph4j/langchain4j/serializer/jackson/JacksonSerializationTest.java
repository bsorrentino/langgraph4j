...
public class JacksonSerializationTest {
	...
	public void StateSerializerTest() throws Exception {
		...
		var state = new State(Map.of("system", SystemMessage.from("Buddy"), "user", UserMessage.from("Hello")));
	...
	public void MessagesStateSerializerTest() throws Exception {
		...
		var state = new State(Map.of("messages", List.of(SystemMessage.from("Buddy"), UserMessage.from("Hello")),
				"intent", "myIntent"));
	...
	public void AiMessageSerializerTest01() throws Exception {
		...
		var toolRequest1 = ToolExecutionRequest.builder().id("id1").name("name1").arguments("arguments1").build();
		var toolRequest2 = ToolExecutionRequest.builder().name("name2").arguments("arguments2").build();
		var aiMessage = new AiMessage.Builder().text("")
			.toolExecutionRequests(List.of(toolRequest1, toolRequest2))
			.build();

		var state = new State(Map.of("messages", List.of(aiMessage)));
	...
	public void AiMessageSerializerTest02() throws Exception {
		...
		var aiMessage = new AiMessage.Builder().text("My text").toolExecutionRequests(List.of()).build();
