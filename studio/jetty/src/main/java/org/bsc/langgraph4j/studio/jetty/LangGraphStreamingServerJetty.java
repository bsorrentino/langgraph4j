...
/**
 * Represents a streaming server for LangGraph using Jetty. LangGraphStreamingServer is an
 * interface that represents a server that supports streaming of LangGraph.
 * Implementations of this interface can be used to create a web server that exposes an
 * API for interacting with compiled language graphs.
 */
	...
	public static class Builder {
		...
		public LangGraphStreamingServerJetty build() throws Exception {
			...
			if (compileConfig.checkpointSaver().isEmpty()) {
				compileConfig = CompileConfig.builder(compileConfig).checkpointSaver(new MemorySaver()).build();
			}
			...
			context.addServlet(new ServletHolder(new GraphStreamServlet(stateGraph, compileConfig, inputArgs)),
					"/stream");
