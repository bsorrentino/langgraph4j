...
/**
 * Interface for a LangGraph Streaming Server. Provides methods to start the server and
 * manage streaming of graph data.
 */
public interface LangGraphStreamingServer {
	...
	class GraphStreamServlet extends HttpServlet {
		 ...
		 */
		public GraphStreamServlet(StateGraph<? extends AgentState> stateGraph, CompileConfig compileConfig,
			...
			if (stateGraph
				.getStateSerializer() instanceof JacksonStateSerializer<? extends AgentState> jsonSerializer) {
		...
		protected void doPost(HttpServletRequest request, HttpServletResponse response)
			...
			try {
				...
				var dataMap = candidateDataMap.entrySet().stream().map(entry -> {
					var newValue = args.stream()
						.filter(arg -> arg.name().equals(entry.getKey()) && arg.converter() != null)
						.findAny()
						.map(arg -> arg.converter.apply(entry.getValue()));
					return newValue.map(v -> entryOf(entry.getKey(), v)).orElse(entry);
	 ...
	 */
	record ArgumentMetadata(String name, ArgumentType type, boolean required,
	 ...
	 */
	record InitData(String title, String graph, List<ArgumentMetadata> args, List<ThreadEntry> threads) {
	...
	class GraphInitServlet extends HttpServlet {
		...
		public void init(ServletConfig config) throws ServletException {
			...
			try {
				...
				var graph = compiledGraph.getGraph(GraphRepresentation.Type.MERMAID,
						/* initData.title() */ null, false);
...
/**
 * Serializer for NodeOutput objects, extending the StdSerializer class. This class is
 * responsible for converting NodeOutput instances into JSON format.
 */
...
class NodeOutputSerializer extends StdSerializer<NodeOutput> {
	...
	/**
	 * Constructs a new NodeOutputSerializer. Calls the superclass constructor with the
	 * NodeOutput class type.
	 */
	...
	public void serialize(NodeOutput nodeOutput, JsonGenerator gen, SerializerProvider serializerProvider)
		...
		// serializerProvider.defaultSerializeField("state", nodeOutput.state().data(),
		// gen);
