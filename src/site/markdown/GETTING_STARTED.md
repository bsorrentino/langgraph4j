# Getting Started with LangGraph4j
...
class GreeterNode implements NodeAction<SimpleState> {
    ...
    public Map<String, Object> apply(SimpleState state) {
        ...
        return Collections.singletonMap(SimpleState.MESSAGES_KEY, "Hello from GreeterNode!");
    }
}
...
class ResponderNode implements NodeAction<SimpleState> {
    ...
    public Map<String, Object> apply(SimpleState state) {
        ...
        if (currentMessages.contains("Hello from GreeterNode!")) {
            return Collections.singletonMap(SimpleState.MESSAGES_KEY, "Acknowledged greeting!");
        }
        return Collections.singletonMap(SimpleState.MESSAGES_KEY, "No greeting found.");
    }
}
```
...
public class SimpleGraphApp {
    public static void main(String[] args) {
        ...
        stateGraph.addNode("greeter", greeterNode);
        stateGraph.addNode("responder", responderNode);

        // Define edges
        stateGraph.addEdge(Node.START, "greeter"); // Start with the greeter node
        stateGraph.addEdge("greeter", "responder");
        stateGraph.addEdge("responder", Node.END);   // End after the responder node

        // Compile the graph
        Graph<SimpleState> compiledGraph = stateGraph.compile();

        // Prepare initial input for the graph
        // We start with an empty state (no initial messages)
        SimpleState initialState = new SimpleState(Collections.emptyMap());
        ...
        for (var item : compiledGraph.stream(initialState, null)) {
            finalState = item;
        }
        ...
        if (finalState != null) {
            ...
            System.out.println("Graph execution did not produce a final state.");
        }
    }
}
```
...
*   **Graph Visualization:** Generate [PlantUML](https://plantuml.com/) or [Mermaid](https://mermaid.js.org/) diagrams of your graph to visualize its structure, which aids in understanding and debugging. See `how-tos/plantuml.ipynb`.
...
*   **Study the Samples:** Check out the [`samples/`](https://github.com/langgraph4j/langgraph4j/tree/main/samples) directory for more complete application examples, including integrations with Langchain4j and Spring AI.
