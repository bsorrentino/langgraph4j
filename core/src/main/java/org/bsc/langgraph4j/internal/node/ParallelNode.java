...
public class ParallelNode<State extends AgentState> extends Node<State> {
	...
	record AsyncParallelNodeAction<State extends AgentState>(List<AsyncNodeActionWithConfig<State>> actions,
		...
		private CompletableFuture<Map<String, Object>> evalGenerator(AsyncGenerator<NodeOutput<State>> generator,
				Map<String, Object> initPartialState) {
		...
		private CompletableFuture<Map<String, Object>> evalNodeAction(AsyncNodeActionWithConfig<State> action,
				State state, RunnableConfig config) {
			return action.apply(state, config)
				.thenCompose(partialState -> partialState.entrySet()
					.stream()
					...
					.map(generatorEntry -> {
						...
						return evalGenerator((AsyncGenerator<NodeOutput<State>>) generatorEntry.getValue(),
		...
		public CompletableFuture<Map<String, Object>> apply(State state, RunnableConfig config) {
			...
			return actions.stream()
				...
				.reduce(completedFuture(new ConcurrentHashMap<>()), (futureResult, futureActionResult) -> futureResult
					.thenCombine(futureActionResult, (result, actionResult) -> AgentState.updateState(result,
							actionResult,
							channels)) /*
