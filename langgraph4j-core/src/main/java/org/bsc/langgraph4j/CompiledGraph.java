...
import org.bsc.langgraph4j.internal.node.CommandNode;
import org.bsc.langgraph4j.internal.node.ParallelNode;
import org.bsc.langgraph4j.state.AgentState;
import org.bsc.langgraph4j.state.StateSnapshot;

/**
 * Represents a compiled graph of nodes and edges. This class manage the StateGraph execution
 *
 * @param <State> the type of the state associated with the graph
 */
public class CompiledGraph<State extends AgentState> {
  private static final org.slf4j.Logger log =
      org.slf4j.LoggerFactory.getLogger(CompiledGraph.class);
  ...
  protected CompiledGraph(StateGraph<State> stateGraph, CompileConfig compileConfig)
    ...
    for (var e : processedData.edges().elements) {
      ...
      if (targets.size() == 1) {
        ...
        var parallelNodeTargets =
            parallelNodeEdges.stream().map(ee -> ee.target().id()).collect(Collectors.toSet());

        if (parallelNodeTargets.size() > 1) {

          var conditionalEdges =
              parallelNodeEdges.stream().filter(ee -> ee.target().value() != null).toList();
          if (!conditionalEdges.isEmpty()) {
            throw StateGraph.Errors.unsupportedConditionalEdgeOnParallelNode.exception(
                e.sourceId(), conditionalEdges.stream().map(Edge::sourceId).toList());
          }
          throw StateGraph.Errors.illegalMultipleTargetsOnParallelNode.exception(
  ...
  /**
   * Gets the history of graph states relate to a specific Thread ID. Useful for: - Debugging
   * execution history - Implementing time travel - Analyzing graph behavior
   *
   * @param config the RunnableConfig containing the thread ID information
   * @return collection of StateSnapshots of the given Thread ID. The first element of collection is
   *     the last state
   */
  ...
  /**
   * Same of {@link #stateOf(RunnableConfig)} but throws an IllegalStateException if checkpoint is
   * not found.
   *
  ...
  /**
   * Update the state of the graph with the given values. If asNode is given, it will be used to
   * determine the next node to run. If not given, the next node will be determined by the state
   * graph.
   *
   ...
   */
  public RunnableConfig updateState(
      RunnableConfig config, Map<String, Object> values, String asNode) throws Exception {
    BaseCheckpointSaver saver =
        compileConfig
            .checkpointSaver()
            .orElseThrow(() -> (new IllegalStateException("Missing CheckpointSaver!")));
    ...
    if (asNode != null) {
      ...
      branchCheckpoint =
          branchCheckpoint.updateState(nextNodeCommand.update(), stateGraph.getChannels());
    }
  ...
  private Command nextNodeId(
      EdgeValue<State> route, Map<String, Object> state, String nodeId, RunnableConfig config)
      throws Exception {
    ...
    throw RunnableErrors.executionError.exception(
        format("invalid edge value for nodeId: [%s] !", nodeId));
  }
   ...
   */
  private Command nextNodeId(String nodeId, Map<String, Object> state, RunnableConfig config)
      throws Exception {
  ...
  private Optional<Checkpoint> addCheckpoint(
      RunnableConfig config, String nodeId, Map<String, Object> state, String nextNodeId)
      throws Exception {
        ...
        .orElseGet(
            () ->
                AgentState.updateState(
                    getInitialStateFromSchema(), inputs, stateGraph.getChannels()));
  }
   ...
   */
  public AsyncGenerator<NodeOutput<State>> stream(
      Map<String, Object> inputs, RunnableConfig config) {
    Objects.requireNonNull(config, "config cannot be null");
    final AsyncNodeGenerator<NodeOutput<State>> generator =
        new AsyncNodeGenerator<>(inputs, config);
   ...
   * @return an AsyncGenerator stream of NodeOutput
   */
  public AsyncGenerator<NodeOutput<State>> streamSnapshots(
      Map<String, Object> inputs, RunnableConfig config) {
    Objects.requireNonNull(config, "config cannot be null");

    final AsyncNodeGenerator<NodeOutput<State>> generator =
        new AsyncNodeGenerator<>(inputs, config.withStreamMode(StreamMode.SNAPSHOTS));
    return new AsyncGenerator.WithEmbed<>(generator);
  }
  ...
  public GraphRepresentation getGraph(
      GraphRepresentation.Type type, String title, boolean printConditionalEdges) {

    String content =
        type.generator.generate(
            processedData.nodes(), processedData.edges(), title, printConditionalEdges);
  ...
  public GraphRepresentation getGraph(GraphRepresentation.Type type, String title) {
    ...
    String content =
        type.generator.generate(processedData.nodes(), processedData.edges(), title, true);
  ...
  public class AsyncNodeGenerator<Output extends NodeOutput<State>>
      ...
      if (isResumeRequest) {
        ...
        var startCheckpoint =
            saver
                ...
                .orElseThrow(
                    () ->
                        (new IllegalStateException("Resume request without a saved checkpoint!")));
                    ...
                    data -> {
                      if (data != null) {
                        ...
                        if (data instanceof Map<?, ?>) {
                          // FIX #102
                          // Assume that the whatever used appender channel doesn't accept
                          // duplicates
                          // FIX #104: remove generator
                          var partialStateWithoutGenerator =
                          ...
                          throw new IllegalArgumentException(
                              "Embedded generator must return a Map");
                        }
                      }
          ...
          .thenApply(
              updateState -> {
                try {
                  ...
                  currentState =
                      AgentState.updateState(currentState, updateState, stateGraph.getChannels());
    ...
    private CompletableFuture<Output> evaluateActionWithoutNested(
        AsyncNodeAction<State> action, State withState) {
      ...
      return action
          ...
          .thenApply(
              partialState -> {
                try {
                  currentState =
                      AgentState.updateState(currentState, partialState, stateGraph.getChannels());

                  var nextNodeCommand = nextNodeId(currentNodeId, currentState, config);
                  nextNodeId = nextNodeCommand.gotoNode();
                  currentState = nextNodeCommand.update();

                  Optional<Checkpoint> cp =
    ...
    private CompletableFuture<Output> getNodeOutput() throws Exception {
      ...
      return completedFuture(
          (cp.isPresent() && config.streamMode() == StreamMode.SNAPSHOTS)
              ? buildStateSnapshot(cp.get())
              : buildNodeOutput(currentNodeId));
    }
    ...
    public Data<Output> next() {
      try {
        ...
        if (action == null) throw RunnableErrors.missingNode.exception(currentNodeId);
...
record ProcessedNodesEdgesAndConfig<State extends AgentState>(
  ...
  static <State extends AgentState> ProcessedNodesEdgesAndConfig<State> process(
      StateGraph<State> stateGraph, CompileConfig config) throws GraphStateException {
    ...
    for (var subgraphNode : subgraphNodes) {
      ...
      if (sgEdgeStartTarget.id() == null) {
        throw new GraphStateException(
            format("the target for node '%s' is null!", subgraphNode.id()));
      }
      ...
      for (var edgeWithSubgraphTargetId : edgesWithSubgraphTargetId) {
        ...
        var newEdge =
            edgeWithSubgraphTargetId.withSourceAndTargetIdsUpdated(
                subgraphNode,
                Function.identity(),
                id ->
                    new EdgeValue<>(
                        (Objects.equals(id, subgraphNode.id())
      ...
      if (interruptsAfter.contains(subgraphNode.id())) {
        ...
        var exceptionMessage =
            (edgeWithSubgraphSourceId.target().id() == null)
      ...
      sgEdgesEnd.stream()
          .map(
              e ->
                  e.withSourceAndTargetIdsUpdated(
                      subgraphNode,
                      subgraphNode::formatId,
                      id ->
                          (Objects.equals(id, END)
