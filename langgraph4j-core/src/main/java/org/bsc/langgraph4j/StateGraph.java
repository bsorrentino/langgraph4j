...
public class StateGraph<State extends AgentState> {
  ...
  public StateGraph<State> addNode(
      String id, AsyncCommandAction<State> action, Map<String, String> mappings)
      throws GraphStateException {

    return addNode(id, new CommandNode<>(id, action, mappings));
  }

  private StateGraph<State> addNode(String id, Node<State> node) throws GraphStateException {
   ...
   * @throws GraphStateException if the node identifier is invalid or the node already exists
   */
  public StateGraph<State> addNode(String id, AsyncNodeActionWithConfig<State> action)
      throws GraphStateException {
  ...
  /**
   * Adds a subgraph to the state graph by creating a node with the specified identifier. This
   * implies that Subgraph share the same state with parent graph
   *
   ...
   */
  @Deprecated
  public StateGraph<State> addSubgraph(String id, CompiledGraph<State> subGraph)
      throws GraphStateException {
   ...
   *     exists @Deprecated use {@code add( String id, StateGraph<State> )} instead
   */
  @Deprecated
  public StateGraph<State> addSubgraph(String id, StateGraph<State> subGraph)
      throws GraphStateException {
    return addNode(id, subGraph);
  }
   ...
   * @throws GraphStateException if the edge identifier is invalid, the mappings are empty, or the
   *     edge already exists
   */
  public StateGraph<State> addConditionalEdges(
      String sourceId, AsyncCommandAction<State> condition, Map<String, String> mappings)
      throws GraphStateException {
    if (Objects.equals(sourceId, END)) {
      throw Errors.invalidEdgeIdentifier.exception(END);
    }
   ...
   */
  public StateGraph<State> addConditionalEdges(
      String sourceId, AsyncEdgeAction<State> condition, Map<String, String> mappings)
      throws GraphStateException {
  ...
  public enum Errors {
    ...
    illegalMultipleTargetsOnParallelNode(
        "parallel node [%s] must have only one target, but %s have been found!"),
    ...
    public GraphStateException exception(Object... args) {
      return new GraphStateException(format(errorMessage, args));
    }
  }
