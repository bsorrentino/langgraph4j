...
/**
 * Class to grade answers based on whether they address a given question. Implements the
 * Function interface to take in Arguments and output a Score.
 */
public record AnswerGrader(String openApiKey) implements Function<AnswerGrader.Arguments, AnswerGrader.Score> {
	...
	/**
	 * Represents the arguments for a structured prompt, encapsulating both a user's
	 * question and an LLM-generated response.
	 */
	...
	interface Service {
		...
		/**
		 * Evaluates if the provided user message addresses and/or resolves the given
		 * question.
		 * @param userMessage The user's input that needs to be evaluated.
		 * @return A binary score indicating whether the user message resolves the
		 * question ('yes') or not ('no').
		 */
