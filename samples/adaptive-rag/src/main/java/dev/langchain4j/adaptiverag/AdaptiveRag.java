...
public class AdaptiveRag {
	...
	/**
	 * Represents the state of our graph. Attributes: question: question generation: LLM
	 * generation documents: list of documents
	 */
	 ...
	 *
	 * This constructor initializes the AdaptiveRag instance by validating the provided
	 * API keys. Both keys are required for the functionality of this class. If either key
	 * is null, an exception is thrown. This ensures that the AdaptiveRag instance is
	 * always properly configured.
	 */
	public AdaptiveRag(String openApiKey, String tavilyApiKey) {
		...
		// The ChromaStore instance is lazily initialized when accessed via the
		// getChroma() method.
	}
	...
	private Map<String, Object> retrieve(State state) {
		...
		List<String> documents = relevant.matches().stream().map(m -> m.embedded().text()).toList();
	...
	private Map<String, Object> gradeDocuments(State state) {
		...
		List<String> filteredDocs = documents.stream().filter(d -> {
	...
	/**
	 * Edge: Determines whether the generation is grounded in the document and answers
	 * question.
	 * @param state The current graph state
	...
	public StateGraph<State> buildGraph() throws Exception {
		return new StateGraph<>(State::new)
			...
			.addNode("grade_documents", node_async(this::gradeDocuments)) // grade
																			// documents
			...
			.addConditionalEdges("grade_documents", edge_async(this::decideToGenerate),
					Map.of("transform_query", "transform_query", "generate", "generate"))
			.addEdge("transform_query", "retrieve")
			.addConditionalEdges("generate", edge_async(this::gradeGeneration_v_documentsAndQuestion),
					Map.of("not supported", "generate", "useful", END, "not useful", "transform_query"));
	}
	...
	public static void main(String[] args) throws Exception {
		...
		org.bsc.async.AsyncGenerator<org.bsc.langgraph4j.NodeOutput<State>> result = graph
			.stream(Map.of("question", "What player at the Bears expected to draft first in the 2024 NFL draft?"));
		// var result = graph.stream( Map.of( "question", "What kind the agent memory do
		// iu know?" ) );
