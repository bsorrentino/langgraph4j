...
public class AgentPayment extends AbstractAgentExecutor<AgentPayment.Builder> {
	...
	static class Tools {
		...
		record Transaction(@Description("the product name bought") String product,
				@Description("code operation") String code) {
		}

		@Tool("submit a payment for purchasing a specific product")
		Transaction submitPayment(@P("the product name to buy") String product, @P("the product price") double price,
