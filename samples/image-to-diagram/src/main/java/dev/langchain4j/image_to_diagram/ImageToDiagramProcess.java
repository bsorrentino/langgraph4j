...
public class ImageToDiagramProcess implements ImageToDiagram {
	 ...
	 *
	 * The {@code ImageUrlOrData} record is designed to hold either a URL pointing to an
	 * image or the image data itself. It provides two static factory methods for creating
	 * instances: one from a URL and another from the image data.
	 *
	 * @param url the URI representing the image URL
	 * @param data the image data as a string
	 */
	public record ImageUrlOrData(URI url, String data) {
		/**
		 * Creates an instance of {@code ImageUrlOrData} from a given URL.
		 * @param url the URI representing the image URL
		 * @return a new instance of {@code ImageUrlOrData} with the specified URL and no
		 * data
		 */
		...
		/**
		 * Creates a new instance of the {@code ImageUrlOrData} class with the provided
		 * data.
		 * @param data image data as a string
		 * @return a new {@code ImageUrlOrData} instance containing the specified image
		 * data
		 */
	...
	final AsyncNodeAction<State> describeDiagramImage = node_async(new DescribeDiagramImage(getVisionModel()));

	final AsyncNodeAction<State> translateSequenceDiagramToPlantUML = node_async(
			new TranslateSequenceDiagramToPlantUML(getModel()));

	final AsyncNodeAction<State> translateGenericDiagramToPlantUML = node_async(
			new TranslateGenericDiagramToPlantUML(getModel()));
	...
	/**
	 * Generates a workflow graph using the StateGraph framework with specific nodes and
	 * conditional edges.
	...
	public StateGraph<State> workflow() throws Exception {
		...
		return new StateGraph<>(State.SCHEMA, stateSerializer).addNode("agent_describer", describeDiagramImage)
			...
			.addConditionalEdges("agent_describer", routeDiagramTranslation,
					Map.of("sequence", "agent_sequence_plantuml", "generic", "agent_generic_plantuml"))
	...
	/**
	 * This method orchestrates a workflow for processing diagrams. It initializes a state
	 * graph and adds nodes and subgraphs to represent different stages of diagram
	 * correction and translation. The method also sets up conditional edges based on the
	 * type of diagram being translated.
	...
	/**
	 * Executes the workflow using the provided image data and returns a stream of
	 * NodeOutput objects.
