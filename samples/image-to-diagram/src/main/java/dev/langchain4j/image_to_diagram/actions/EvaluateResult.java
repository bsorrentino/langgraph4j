 ...
 * specifically targeting the conversion of image data to diagram data using an AI chat
 * model. It implements the AsyncNodeAction interface, which allows for asynchronous
 * processing of a State object into a Map. The class leverages the OpenAiChatModel for
 * its operations.
 */
public class EvaluateResult implements AsyncNodeAction<ImageToDiagram.State> {
	 ...
	 * {@link DiagramCorrectionProcess}. This method processes the input state
	 * asynchronously and returns a {@link CompletableFuture} with the resulting diagram
	 * data. If no results are generated, it throws a {@link RuntimeException}.
	 * @param state The input state for the diagram correction process.
	 * @return A {@link CompletableFuture} that will contain the diagram data upon
	 * successful completion, or be completed exceptionally if an error occurs during the
	 * processing.
	 */
	@Override
	public CompletableFuture<Map<String, Object>> apply(ImageToDiagram.State state) {
		...
		try {
			return diagramCorrectionProcess.workflow()
				.compile()
				.stream(state.data())
