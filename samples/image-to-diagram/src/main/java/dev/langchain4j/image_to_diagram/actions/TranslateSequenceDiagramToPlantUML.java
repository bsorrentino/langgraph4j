...
/**
 * The `TranslateSequenceDiagramToPlantUML` class implements the
 * {@literal NodeAction<ImageToDiagram.State>} interface. It is responsible for
 * translating a sequence diagram into PlantUML code using an OpenAI chat model.
 */
public class TranslateSequenceDiagramToPlantUML implements NodeAction<ImageToDiagram.State> {

	/**
	 * The `model` field represents the OpenAiChatModel used for generating PlantUML code
	 ...
	 *
	 * This method processes the given state containing an image, generates a system
	 * prompt, and uses a language model to produce PlantUML code representing the
	 * diagram. If no diagram is provided in the state, it throws an
