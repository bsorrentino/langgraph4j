...
/**
 * The ReviewResult class implements the AsyncNodeAction interface for processing
 * ImageToDiagram.State objects. It handles the review of diagram code and error messages
 * using a language model from OpenAiChatModel.
 */
public class ReviewResult implements AsyncNodeAction<ImageToDiagram.State> {
	...
	/**
	 * The final instance variable holding the OpenAiChatModel. It is initialized via the
	 * constructor.
	 */
	 ...
	 * @param state The ImageToDiagram.State containing diagram code and evaluation
	 * errors.
	 * @return A CompletableFuture containing the review result or an exception if
	 * processing fails.
	 */
	@Override
	public CompletableFuture<Map<String, Object>> apply(ImageToDiagram.State state) {
		...
		try {
			...
			/*
			 * Retrieves the diagram code from the state or throws an
			...
			/*
			 * Retrieves the evaluation error from the state or throws an
			 * IllegalArgumentException if not available.
			 */
			...
			/*
			 * Loads a prompt template and applies it with the given values for diagram
			 ...
			 */
			var request = ChatRequest.builder().messages(SystemMessage.from(systemPrompt.text())).build();
