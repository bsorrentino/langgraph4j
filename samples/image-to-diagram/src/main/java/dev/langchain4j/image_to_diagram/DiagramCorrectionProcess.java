...
/**
 * Represents the process for correcting diagrams from images using asynchronous node and
 * edge actions.
 */
public class DiagramCorrectionProcess implements ImageToDiagram {
	...
	public StateGraph<State> workflow(StateSerializer<State> stateSerializer) throws Exception {
		return new StateGraph<>(State.SCHEMA, stateSerializer).addNode("evaluate_result", evaluateResult)
			.addNode("agent_review", reviewResult)
			.addEdge("agent_review", "evaluate_result")
			.addConditionalEdges("evaluate_result", routeEvaluationResult,
					Map.of("OK", END, "ERROR", "agent_review", "UNKNOWN", END))
