...
public class JSONStateSerializer extends GsonStateSerializer<ImageToDiagram.State> {
	...
	public JSONStateSerializer() {
		super(ImageToDiagram.State::new,
				new GsonBuilder().registerTypeAdapter(ImageToDiagram.State.class, new StateDeserializer())
					// .registerTypeAdapter(Diagram.Element.class, new
					// ElementDeserializer())
					.create());
	}
...
class StateDeserializer implements JsonDeserializer<ImageToDiagram.State> {
	...
	@Override
	public ImageToDiagram.State deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
		...
		JsonPrimitive evaluationErrorTypeElement = dataElement.getAsJsonObject()
			.getAsJsonPrimitive("evaluationErrorType");
		...
		List<String> diagramCode = gson.fromJson(diagramCodeElement, new TypeToken<List<String>>() {
		}.getType());
		data.put("diagramCode", diagramCode);

		if (diagramElement != null) {
			var diagram = gson.fromJson(diagramElement, new TypeToken<Diagram.Element>() {
			}.getType());
			data.put("diagram", diagram);
		}

		if (evaluationResultElement != null) {
			var evaluationResult = gson.fromJson(evaluationResultElement,
					new TypeToken<ImageToDiagram.EvaluationResult>() {
					}.getType());
		...
		if (evaluationErrorTypeElement != null) {
			var evaluationErrorType = gson.fromJson(evaluationErrorTypeElement, new TypeToken<ErrorUmlType>() {
			}.getType());
...
class ElementDeserializer implements JsonDeserializer<Diagram.Element> {
	...
	@Override
	public Diagram.Element deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
			throws JsonParseException {
		...
		var gson = new Gson();
		List<Diagram.Participant> participants = gson.fromJson(participantsElement,
				new TypeToken<List<Diagram.Participant>>() {
				}.getType());
		List<Diagram.Relation> relations = gson.fromJson(relationsElement, new TypeToken<List<Diagram.Relation>>() {
		}.getType());
		List<Diagram.Container> containers = gson.fromJson(containersElement, new TypeToken<List<Diagram.Container>>() {
		}.getType());
		List<String> description = gson.fromJson(descriptionElement, new TypeToken<List<String>>() {
		}.getType());
