...
public class RouteDiagramTranslation implements AsyncEdgeAction<ImageToDiagram.State> {
	...
	/**
	 * Applies the transformation to convert an {@code ImageToDiagram.State} to a diagram
	 * type. If the state's diagram type is "sequence" (case-insensitive), it returns
	 * "sequence"; otherwise, it returns "generic".
	 * @param state The current state of the image-to-diagram conversion process.
	 * @return A {@link CompletableFuture} that completes with the determined diagram type
	 * as a string.
	 */
	@Override
