...
/**
 * Class declaration for EvaluateResult, which implements the AsyncNodeAction interface
 * for ImageToDiagram.State. This class is responsible for evaluating a diagram code and
 * returning an evaluation result wrapped in a CompletableFuture.
 */
public class EvaluateResult implements AsyncNodeAction<ImageToDiagram.State> {
	 ...
	 * @param state The current state of ImageToDiagram, containing the diagram code to be
	 * evaluated.
	 * @return A CompletableFuture that resolves with a map containing the evaluation
	 * result.
	 */
	@Override
		...
		return PlantUMLAction.validate(diagramCode)
			...
			.exceptionally(e -> {
				if (e.getCause() instanceof PlantUMLAction.Error) {
					return Map.of("evaluationResult", ImageToDiagram.EvaluationResult.ERROR, "evaluationError",
							e.getCause().getMessage(), "evaluationErrorType",
							((PlantUMLAction.Error) e.getCause()).getType());
				}
