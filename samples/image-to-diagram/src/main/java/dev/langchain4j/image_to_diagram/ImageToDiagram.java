...
public interface ImageToDiagram {
	...
	/**
	 * Represents the state of an agent, specifically tailored for managing
	 * diagram-related data and processes.
	 */
	class State extends AgentState {

		static Map<String, Channel<?>> SCHEMA = Map.of("diagramCode", Channels.appender(ArrayList::new));
		 ...
		 * @return An {@link Optional} containing the image data, or empty if not
		 * available.
		 */
		 ...
		 * @return an {@link Optional} containing the diagram element if it exists,
		...
		/**
		 * Returns a list of diagram code. If the "diagramCode" value is not present,
		 ...
		 * {@link ImageToDiagramProcess.EvaluationResult}.
		 * @return An {@link Optional} containing the evaluation result, or empty if not
		 * available.
		 */
		...
		/**
		 * Returns an {@link Optional} containing the value associated with
		 * "evaluationError".
		 * @return an {@link Optional} containing the value associated with
		 * "evaluationError"
		 */
		 ...
		 * @return an {@link Optional} containing the error UML type, or an empty optional
		 * if not available
		 */
		...
		public boolean isExecutionError() {
			return evaluationErrorType().map(type -> type == ErrorUmlType.EXECUTION_ERROR).orElse(false);
		}
		...
		public boolean lastTwoDiagramsAreEqual() {
			...
			String last = last(diagramCode()).map(String::trim)
				.orElseThrow(() -> new IllegalStateException("last() is null!"));
			String prev = lastMinus(diagramCode(), 1).map(String::trim)
	 ...
	 * @return An {@link OpenAiChatModel} object with predefined settings suitable for
	 * processing images and videos.
	 * @throws IllegalArgumentException if no OPENAI_API_KEY is provided in the system
	 * properties.
	 */
	 ...
	 * @return a configured {@link OpenAiChatModel} with the API key obtained from the
	 * system property "OPENAI_API_KEY".
	 * @throws IllegalArgumentException if no OPENAI_API_KEY is provided as a system
	 * property.
	 */
