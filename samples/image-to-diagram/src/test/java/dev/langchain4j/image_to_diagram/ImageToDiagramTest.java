...
public class ImageToDiagramTest {
	...
	public void describeDiagramImage() throws Exception {
		...
		var imageUrlOrData = ImageToDiagramProcess.ImageUrlOrData
			.of(new URI("https://blog.langchain.dev/content/images/2024/01/supervisor-diagram.png"));
		...
		var request = ChatRequest.builder().messages(message).build();
	...
	public void imageToDiagram() throws Exception {
		...
		var diagramCode = result.stream()
			.reduce((out1, out2) -> out2)
	...
	private void reviewDiagram(String diagramId) throws Exception {
		...
		var result = process.workflow()
			.compile()
			.stream(Map.of("diagramCode", diagramCode))
	...
	public void getGraph() throws Exception {
		...
		var plantUml = agentExecutor.workflow()
			.getGraph(GraphRepresentation.Type.PLANTUML, "Image to diagram with correction", false);
		...
		var mermaid = agentExecutor.workflow()
			.getGraph(GraphRepresentation.Type.MERMAID, "Image to diagram with correction", false);
		...
		var correctionPlantUml = correctionProcess.workflow()
			.getGraph(GraphRepresentation.Type.MERMAID, "Correction Process", false);
