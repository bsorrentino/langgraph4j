...
public class Issue102Test {
	...
	public void testWithStream() throws GraphStateException {
		...
		var graph = new StateGraph<>(AgentState::new)
			...
			.addNode("node2", node_async(agentState -> Map.of("node2_result", new AsyncGenerator<StreamingOutput<?>>() {
				...
				public Data<StreamingOutput<?>> next() {
					...
					if (i == 3) {
						...
						// return Data.done( mergeMap(agentState.data(),
						// Map.of("node2_stream_result", sb.toString())) );
					}
		...
		assertIterableEquals(List.of("node1_result", "input", "node3_result", "node2_stream_result"),
	...
	public void testWithSubgraph() throws GraphStateException {
		...
		assertTrue(result.isPresent());
		assertIterableEquals(List.of("node1_result", "input", "node3_result", "sb:node1_result"),
				result.get().data().keySet());
