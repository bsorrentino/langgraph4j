...
public class MCPIntegrationITest {
	...
	public void testMCPClientCall() throws Exception {
		...
		var transport = new StdioMcpTransport.Builder()
			.command(List.of("docker", "run", "-i", "--rm", "mcp/postgres",
		...
		try (var mcpClient = new DefaultMcpClient.Builder().transport(transport).build()) {

			var dbTableRes = mcpClient.listResources().stream().toList();
			...
			for (var i = 0; i < dbTableRes.size(); ++i) {
				...
				context.append(dbTableRes.get(i).name()).append(" = ").append(dbColumnsRes.get(i)).append("\n\n");

			}

			var agentBuilder = AgentExecutor.builder().chatModel(model);
			...
			var message = prompt.apply(Map.of("schema", context, "input", "get all issues names and project"))
				.toUserMessage();
